...
import { refreshAccountData, signIn } from "./apiService"
...
class AccountStorageService {
  ...
  async signInAccount(
    ...
    try {
      ...
      if (result.success) {
        await this.updateSignInDate(id)
        console.log(`账号 ${account.site_name} 签到成功:`, result.message)
      } else if (result.isAlreadySigned) {
        await this.updateSignInDate(id)
        console.log(`账号 ${account.site_name} 今日已签到`)
      } else {
        console.warn(`账号 ${account.site_name} 签到失败:`, result.message)
      }
  ...
  convertToDisplayData(accounts: SiteAccount[]): DisplaySiteData[] {
    ...
    return accounts.map((account) => ({
      ...
      signedToday: account.lastSignInDate === todayTimestamp
    }))
  }
   ...
   * 更新账号签到日期
   */
  async updateSignInDate(id: string): Promise<boolean> {
    const today = new Date()
    today.setHours(0, 0, 0, 0)
    const todayTimestamp = today.getTime()

    return this.updateAccount(id, { lastSignInDate: todayTimestamp })
  }
