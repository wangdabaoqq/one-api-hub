...
export function useUserPreferences() {
  ...
  // 更新打开插件时自动签到设置
  const updateAutoSignInOnOpen = useCallback(
    async (autoSignInOnOpen: boolean) => {
      try {
        const success =
          await userPreferences.updateAutoSignInOnOpen(autoSignInOnOpen)
        if (success && preferences) {
          setPreferences((prev) =>
            prev ? { ...prev, autoSignInOnOpen } : null
          )
          console.log(
            "[useUserPreferences] 打开插件时自动签到设置更新成功:",
            autoSignInOnOpen
          )
        }
        return success
        ...
        console.error(
          "[useUserPreferences] 更新打开插件时自动签到设置失败:",
          error
        )
        return false
      }
  ...
  return {
    ...
    autoSignInOnOpen: preferences?.autoSignInOnOpen ?? true,
